...
document.getElementById("submitButton").onclick = function (ev) {
    ...
    if (!p || !q)
        return errorElement.textContent = "Invalid input!";
    const values = pq(p, q);
    if (!values)
        return errorElement.textContent = "Invalid numbers!";
    if (values.firstValue == values.secondValue)
        createElement(pElementOnHTML, values.firstValue, "The value is");
    else {
...
function createElement(pElement, value, textContent) {
...
;
//# sourceMappingURL=pqWithImport.js.map